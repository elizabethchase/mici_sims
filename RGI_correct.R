...
comp_times <- u$ftime[u$ftype==2]
cens_times <- u$ftime[u$ftype==0]
critical_ties <- intersect(comp_times, cens_times)

ints <- diff(sort(u$ftime))
smallest_int <- min(ints[ints>0])/2
if (length(critical_ties)>0){
  u$ftime[u$ftype==0 & u$ftime %in% critical_ties] <- u$ftime[u$ftype==0 & u$ftime %in% critical_ties] + smallest_int
}
